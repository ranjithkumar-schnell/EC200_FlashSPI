...
void set_gps_time()
{
	ql_rtc_get_time(&g_dt_tm);
	ql_rtc_set_timezone(22);    //UTC+32
	ql_rtc_get_localtime(&g_dt_tm);
	sprintf(GpsDate_time_CRI,"$D%02d/%02d/%02d%02d.%02d.%02d#",g_dt_tm.tm_mday,g_dt_tm.tm_mon,g_dt_tm.tm_year,g_dt_tm.tm_hour,g_dt_tm.tm_min,g_dt_tm.tm_sec);
	sprintf(current_dt_gps,"%04d-%02d-%02d %02d:%02d:%02d",g_dt_tm.tm_year,g_dt_tm.tm_mon,g_dt_tm.tm_mday,g_dt_tm.tm_hour,g_dt_tm.tm_min,g_dt_tm.tm_sec);
	nmea_dbg_log("=========print  gps set time :%s===========\r\n ",GpsDate_time_CRI);
	nmea_dbg_log("=========lat and long value is  : %s ========\r\n ",Lat_Long_CRI);
	nmea_dbg_log("=========date for json payload no ntwrk : %s ========\r\n ",current_dt_gps);
	//nmea_dbg_log("event param1 value from uart is:%d",event_uart.param1);
