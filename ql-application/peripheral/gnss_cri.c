...
ql_event_t event;
ql_queue_t gnss_location_time_Data; // refrence for queue handler


nmeasrc_sentences nmea_handle;
char gnss_device_info[100]={0};
char O_lat[15]={'\0'},O_lon[15]={'\0'},O_sat[15]={'\0'};
char ArrayOfString[25][15]={'\0'};
bool gpsloc=false;
char delims[] = "/:,{}\"+$";
static uint32 prev_ms_gsv =0;
static bool tick_overflow = FALSE;
char GpsDate_time_CRI[25]={'\0'};
char Lat_Long_CRI[25]={'\0'};
//char current_dt_gps[25]={'\0'};
uint32 gps_Set_event = 0;
...
int SeperateSring(char* string)
{
	int err,j;
	float res;
	char *p;
	char t_lat[10]={'\0'},tmp1[10]={'\0'};   
	      
  	QL_GNSSDEMO_LOG("SeperateSring=%s\r\n",string);
	//QL_GNSSDEMO_LOG("\r\nArrayOfString[%d] :%s\r\n",i,ArrayOfString[i]);
  	p = strtok (string,delims);
  	memset(ArrayOfString, 0, sizeof(ArrayOfString)); 
  	int i = 0;
  	while (p!= NULL)
	{
    	strcpy(ArrayOfString[i], p);
		QL_GNSSDEMO_LOG("\r\nGGA Value after tokenisation[%d] :%s\r\n",i,ArrayOfString[i]);
    	p = strtok (NULL, delims);
		i++;
  	}
	// if(strcmp(ArrayOfString[1],"A")==0)
	if(atoi(ArrayOfString[5]))   // gets the gps set data refere nmea gga format.
	{

		gpsloc = true;
		gps_Set_event = 1;
		//QL_GNSSDEMO_LOG("gps location locked:%d \r\n",event.param3);
		strcpy(t_lat,ArrayOfString[1]);
		j=(int)atoi(t_lat)/100;
		for (int k=2,l=0;t_lat[k]!='\0';k++,l++)
    	{
        tmp1[l]=t_lat[k];
    	}
		res=(j+(atof(tmp1)/60));
		QL_GNSSDEMO_LOG("\r\n====The latlong value is :%.6f====\r\n",res);
		if(res >1)QL_GNSSDEMO_LOG("\r\n====THE GPS IS SET=======\r\n");
		else QL_GNSSDEMO_LOG("\r\n====THE GPS IS not  SET=======\r\n");
		sprintf(O_lat,"%.6f",res);

		res=0;


		strcpy(t_lat,ArrayOfString[3]);
		j=(int)atoi(t_lat)/100;
		for (int k=3,l=0;t_lat[k]!='\0';k++,l++)
    	{
        tmp1[l]=t_lat[k];
    	}
		res=(j+(atof(tmp1)/60));
		sprintf(O_lon,"%.6f",res);

		strcpy(O_sat,ArrayOfString[6]);
	  	QL_GNSSDEMO_LOG("\r\n filter lat :%s and lon :%s and sat:%s \r\n",O_lat,O_lon,O_sat);
		
		 sprintf(Lat_Long_CRI,"$L%s%s#",O_lat,O_lon);
		QL_GNSSDEMO_LOG("=========lat and long value is  : %s ========\r\n ",Lat_Long_CRI);
         gps_strctdata.lat     		 = O_lat;
		 gps_strctdata.lon      	 = O_lon;
		 gps_strctdata.satlites		 = O_sat;
		 gps_strctdata.latlong_urt   = Lat_Long_CRI;
		 QL_GNSSDEMO_LOG("quue new struct data gps_strctdata.lat :%s",gps_strctdata.lat);
		 QL_GNSSDEMO_LOG("quue new struct data gps_strctdata.lon :%s",gps_strctdata.lon);
		 QL_GNSSDEMO_LOG("quue new struct data gps_strctdata.satlites :%s",gps_strctdata.satlites);
		 QL_GNSSDEMO_LOG("quue new struct data gps_strctdata.date_time_urt :%s",gps_strctdata.latlong_urt);
         ql_rtos_queue_release(gnss_location_time_Data,sizeof(gps_strctdata),(uint8 *)&gps_strctdata,0);
		
		err = 0;
		//ql_rtos_task_delete(NULL);   //for time data   
		//ql_rtos_task_suspend(gnss_task);             
		if(err != QL_OSI_SUCCESS)
		{
			QL_GNSSDEMO_LOG("task deleted failed");
		}
...
void ql_gnss_notify_cb(uint32 ind_type, ql_uart_port_number_e port, uint32 size)
{
    if( ind_type == QUEC_UART_RX_RECV_DATA_IND )
    {
        ...
        event.param1 = port;
        event.param2 = size;
		event.param3 = gps_Set_event;
        ql_rtos_event_send(gnss_task, &event);
		//ql_rtos_event_send(uart_task,&event);
        QL_GNSSDEMO_LOG("gnss demo port is %d, size is %d, gpslock is :%d", event.param1, event.param2,event.param3);
    }
    else
    {
...
static void ql_gnss_demo_thread(void *param)
{
	...
	ql_rtos_queue_create(& gnss_location_time_Data,sizeof(lat_lon_sat_dttm_t),4);// gnss dat to uart sending queue
    while(1)
    {   
...
struct nmea_s* nmea_parse(char *sentence, int length, int check_checksum)
{
	...
	switch(type)
	{
		case NMEA_GGA:
		{
			SeperateSring(val_string);                                                   //only GGA value are considered for the project
			...
			if(ret)
			{
				...
				goto _error;
						   	...
						   	// QL_GNSSDEMO_LOG("gps_data gga -> longitude is :%ld",data->longitude);
							// QL_GNSSDEMO_LOG("gps_data gga-> latitude is :%ld",data->latitude);
		...
		case NMEA_RMC:
		{
			// QL_GNSSDEMO_LOG("gps_data rmc string bef-> longitude is :%s",val_string);
			// SeperateSring(val_string);
			ret = nmea_parse_rmc(nmea, val_string);
			// QL_GNSSDEMO_LOG("gps_data rmc string aft-> longitude is :%s",val_string);
    ...
    switch(nmea->type)
    {
        ...
        case NMEA_GGA:
            ...
            if(gga != NULL)
            {
				...
				nmea_dbg_log("=================DIsplay time in UTC=%d============== \r\n",gps_data->UTC);
				gps_data->time.tm_hour=gps_data->UTC/10000;
				
				g_dt_tm.tm_hour = gps_data->time.tm_hour;//
				nmea_dbg_log("my tm_hour in UTC=%d \r\n", g_dt_tm.tm_hour );
				
				gps_data->time.tm_sec=gps_data->UTC%100;
				
				g_dt_tm.tm_sec = gps_data->time.tm_sec;//
				nmea_dbg_log("my tm_sec in UTC=%d \r\n",g_dt_tm.tm_sec);
				
				gps_data->time.tm_min=(gps_data->UTC/100)%100;
				
				//g_dt_tm.tm_min = gps_data->time.tm_min;//
				g_dt_tm.tm_min = gps_data->time.tm_min;//
				nmea_dbg_log("my tm_min in UTC=%d \r\n",g_dt_tm.tm_min);		
                gps_data->altitude = gga->altitude;

				//set_gps_time();
                
                gps_data->satellites_num=gga->satellites_tracked;
				//if(gpsloc == true)ql_rtos_task_suspend(gnss_task);
            }
        break;
        case NMEA_GSA:
